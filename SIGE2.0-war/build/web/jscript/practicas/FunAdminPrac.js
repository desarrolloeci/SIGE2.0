...
function consulTCampus() {
    var tabla = document.getElementById('consulTCampus');
    var filas = tabla.rows;
    var tam = filas.length;
    for (var i = 1; i < tam; i++) {
        tabla.deleteRow(1);
    }
    var url = 'lisCampusEnvio';
    conectarMethod(url, muestraTCampusEnv, 'POST');
}
...
function  muestraTCampusEnv() {
    ...
    if (http_request.readyState == 4) {
        if (http_request.status == 200) {
            ...
            var lista = xmldoc.getElementsByTagName("listaofertas")[0];
            var reg = lista.getElementsByTagName("oferta");
            var elementos = ["id_oferta", "nit", "cargo", "razon_social", "fecha_limite", "uge_obs", "temp"];
            var cuerpotablaTraoferpracAdmin = document.getElementById('cuerpotablaTraoferpracAdmin'), fila, texto, nodo, nt, estado, columna, div, link;
            var idofer = 0;
            for (var i = 0; i < reg.length; i++) {
                fila = cuerpotablaTraoferpracAdmin.insertRow(i);
...
function muestraRegistrosAdmin() {
    if (http_request.readyState == 4) {
        ...
        if (http_request.status == 200) {
            ...
            var elementos = ["id_documento", "nom_est", "estado", "pendiente_campus"];
            var cuerpotablaPracAdmin = document.getElementById('cuerpotablaPracAdmin'), fila, texto, nodo, nt, estado, columna, div, link;
            var id_doc = 0;
            var idreq = 0;
            var idemp = 0;
...
||!validaFechaCompleta(document.getElementById("fechainPracE").value)||!validaFechaCompleta(document.getElementById("fechaoutPracE").value)||(document.getElementById("perVeriMatri").value).length!==6) {
        return false;
    } else {
...
+ '&fechainPracE=' + document.getElementById("fechainPracE").value+ '&fechaoutPracE=' + document.getElementById("fechaoutPracE").value + '&perVeriMatri=' + document.getElementById("perVeriMatri").value;
