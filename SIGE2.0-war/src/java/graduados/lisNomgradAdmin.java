 ...
 */
public class lisNomgradAdmin extends HttpServlet {
...
//'nombregrad','apellidograd','DocGrad'
           ...
           HashMap[] docs= bd.getLisDocGradAdmin(nombre,apellido,doc,rol);
           
           if(docs.length<=50){
               for (HashMap docu : docs) {
                    String documento=(String) docu.get("documento");
                     //Se consulta la persona en campus
                        HashMap id = emp.docgrad(documento);
                        String emplid = (String)id.get("id_documento");
                        EstudiantesRemote est = conEjb.lookupEstudiantesRemote();
                        est.inicializar("75107740");
                        est.getPersona(emplid, null, "GRADUADO", ConsultaPersonas.consultaPersona(emplid, "GRADUADO"));
                        est.ActDatosEgreDesdeCampus(emplid);
               }
               
               HashMap[] items = bd.getLisNombreGradAdmin(nombre,apellido,doc,rol);
               HashMap item;
               ...
               res="<Nombres><Nombre>" +"<id_est>0</id_est>" + "<nom_est></nom_est>" + "<nom_prog></nom_prog>" +"</Nombre></Nombres>";
           }
