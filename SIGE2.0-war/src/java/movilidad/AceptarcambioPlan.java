 ...
 */
public class AceptarcambioPlan extends HttpServlet {
    ...
    throws ServletException, IOException {
        ...
        try {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet AceptarcambioPlan</title>");  
            out.println("</head>");
            out.println("<body>");
            out.println("<h1>Servlet AceptarcambioPlan at " + request.getContextPath () + "</h1>");
            out.println("</body>");
            out.println("</html>");
            */
        } finally { 
    ...
    throws ServletException, IOException {
       try
        ...
        ret= bd.aceptarcambioplan(idsolicitud);
            //Enviar Respuesta
        res = res + "<dato>" + idsolicitud + "</dato>";
        if (ret > 0) {
                utiles.EnviarEmailRemote em=conEjb.lookupEnviarEmailRemote();
                 em.inicializar(idsolicitud);
                   ban=em.enviarEst(idsolicitud,"CamPlanAcep");
                    if(ban==1){
                      res = res + "<valor>El cambio ha sido aceptado</valor>";
                    }
                   else{
        ...
        //Enviar Respuesta
            ...
            Logger.getLogger(AceptarcambioPlan.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
